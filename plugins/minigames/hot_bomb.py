...
class NewBomb(bs.Actor):
    ...
    def _touch_player(self):
        ...
        if self.bomb_type == 'banana':
            ...
            bs.animate_array(player.actor.node,'color',3,{0:color,0.1:(1.5,1,0),0.5:(1.5,1,0),0.6:color})
...
class Bone(bs.Actor):
    ...
    def __init__(self, position: Sequence[float] = (0, 1, 0),
        ...
        shared = SharedObjects.get()
        meshs = ['bonesUpperArm','bonesUpperLeg','bonesForeArm','bonesPelvis','bonesToes','bonesHand']
        bone = None
        mesh = 0
        for i in meshs:
            if mesh == style:
