	...
	def _handle():
		...
		if len(messages)>1:
			...
			if m == px:
				...
				if n==[]:
					...
					if n[1] in [ 'page' , 'Page'  ]:
						if n[2] == '1':
							...
							cmsg(f' {px}unfreeze/thaw all or {px}unfreeze/thaw number of list | unfreeze the player')
							...
							cmsg(f' {px}fall all or {px}fall number of list | teleport in down and fall up the player')
							cmsg(f' {px}curse all or {px}curse number of list | curse the player')
							cmsg(f' {px}creepy all or {px}creepy number of list | make creepy actor of player')
							cmsg(f' {px}inv all or {px}inv number of list | makes invisible player')
							cmsg(f' {px}celebrate all or {px}celebrate number of list | celebrate action to the player')
							cmsg('============================')
						elif n[2] == '3':
							cmsg('============== Help 2 Page 3 ==============')
							cmsg(f' {px}gm all or {px}gm number of list | give bs gods like powers to player')
							cmsg(f' {px}sp all or {px}sp number of list | give superrrrrrr damages when punch to player')
				...
				if n == []:
					cmsg(f'Use: {px}d_bomb/default_bomb all or {px}d_bomb number of list ,type {px}d_bomb help for help')
				elif n[0]=='help':
				...
				if n == []:
					cmsg(f'Use: {px}d_bomb_count/default_bomb/dbc all or {px}d_bomb_count/default_bomb_count/dbc number of list')
				else:
					try:
						for i in activity_players:
