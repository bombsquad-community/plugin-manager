...
from bauiv1lib.popup import PopupWindow, PopupMenuWindow, PopupMenu
...
def getlanguage(text, subs: str = None, almacen: list = []):
    ...
    translate = {
        "Reset": {"Spanish": "Reiniciar", "English": "Reset", "Portuguese": "Reiniciar"},
        "Nothing": {
            "Spanish": "Sin potenciadores",
            "English": "No powerups",
            "Portuguese": "Sem powerups",
        },
        "Action 1": {"Spanish": "Potenciadores", "English": "Powerups", "Portuguese": "Powerups"},
        "Action 2": {"Spanish": "Configuración", "English": "Settings", "Portuguese": "Definições"},
        "Action 3": {"Spanish": "Extras", "English": "Extras", "Portuguese": "Extras"},
        "Action 4": {"Spanish": "Tienda", "English": "Store", "Portuguese": "Loja"},
        "Action 5": {
            "Spanish": "Canjear código",
            "English": "Enter Code",
            "Portuguese": "Código promocional",
        },
        "Custom": {"Spanish": "", "English": "Customize", "Portuguese": "Customizar"},
        "Impairment Bombs": {
            "Spanish": "Bombas menoscabo",
            "English": "Hyperactive bombs",
            "Portuguese": "Bombas hiperativas",
        },
        "Speed": {"Spanish": "Velocidad", "English": "Speed", "Portuguese": "Velocidade"},
        "Fire Bombs": {
        ...
        "Goodbye": {"Spanish": "¡Hasta luego!", "English": "Goodbye!", "Portuguese": "Adeus!"},
        "Healing Damage": {
        ...
        "SY: BALL": {"Spanish": "Esfera", "English": "Sphere", "Portuguese": "Esfera"},
        "SY: Impact": {"Spanish": "Especial", "English": "Special", "Portuguese": "Especial"},
        "SY: Egg": {"Spanish": "Huevito", "English": "Egg shape", "Portuguese": "Ovo"},
        "Powerup Scale": {
        ...
        "New": {"Spanish": "Nuevo", "English": "New", "Portuguese": "Novo"},
        "Only Bombs": {
            "Spanish": "Sólo Bombas",
            "English": "Only bombs",
        ...
        "Bought": {"Spanish": "Comprado", "English": "Bought", "Portuguese": "Comprou"},
        "Confirm Purchase": {
        ...
        "Timer Store": {"Spanish": 'Temporizador', "English": 'Timer', "Portuguese": 'Timer'},
        "Percentages Store": {"Spanish": 'Extras', "English": 'Extras', "Portuguese": 'Extras'},
        "Block Option Store": {
        ...
        "Creator": {
            "Spanish": "Mod edited by ATD",
            "English": "Mod edited by ATD",
            "Portuguese": "Mod edited by ATD",
        },
        "Mod Info": {
...
def settings_distribution():
    return {
        "Powers Gravity": False,
        "Tank Shield PTG": 96,
        "Healing Damage PTG": 72,
        "Powerup Style": 'Auto',
        "Powerup Scale": 1.0,
        "Powerup Name": False,
        "Powerup With Shield": False,
        "Powerup Time": False,
    }


apg = babase.app.config
if "PPM Settings" in apg:
    old = apg['PPM Settings']
    for settings in settings_distribution():
...
if "Powerups" in config:
    p_old = config['Powerups']
    for powerups in default_powerups():
        if powerups not in p_old:
...
GLOBAL = {"Tab": 'Action 1', "Cls Powerup": 0, "Coins Message": []}


# === STORE ===
def promo_codes():
    return {
        "G-Am54igO42Os": [True, 1100],
...
def store_items():
    return {"Buy Firebombs": True, "Buy Option": True, "Buy Percentage": True}
...
class BearStore:
    def __init__(self, price: int = 1000, value: str = '', callback: call = None):
...
class PromoCode:
    ...
    def code_confirmation(self):
        if self.code != "":
            bs.broadcastmessage(babase.Lstr(resource='submittingPromoCodeText'), (0, 1, 0))
            try:
                babase.pushcall(babase.CallPartial(self.validate_code), from_other_thread=True)
            except:
                pass
        ...
        if self.code in self.codes_store:
            if self.promo_code_expire:
                with babase.ContextRef.empty():
                    babase.pushcall(
                        babase.CallPartial(self.successful_code), from_other_thread=True
                    )
    ...
    def successful_code(self):
        ...
        bs.broadcastmessage(getlanguage('Reward Code', subs=self.promo_code_amount), (0, 1, 0))
...
def new_on_transition_in(self):
    ...
    try:
        if not (STORE['Buy Firebombs'] and STORE['Buy Option'] and STORE['Buy Percentage']):
            ...
            if coins_message != []:
                ...
                if not bear_coin >= (limit - 1):
                    bs.broadcastmessage(getlanguage('Coins Message', subs=result), (0, 1, 0))
                    bui.getsound('cashRegister').play()
                else:
                    bs.broadcastmessage(
                        getlanguage('Coins Limit Message', almacen=[result, limit]), (1, 0, 0)
                    )
                    bui.getsound('error').play()
...
class NewProfileBrowserWindow:
    def __init__(
        ...
        width = 100 if uiscale is babase.UIScale.SMALL else -14
        size = 50
        ...
        if isinstance(self.session, MainMenuSession):
            self.button = bui.buttonwidget(
                ...
                button_type='square',
                label='',
                on_activate_call=babase.CallPartial(self.powerupmanager_window),
            )
            ...
            self.image = bui.imagewidget(
                ...
                texture=bui.gettexture('powerupSpeed'),
            )
...
def fire_effect(self):
    if self.node.exists():
        bs.emitfx(
            position=self.node.position, scale=3, count=50 * 2, spread=0.3, chunk_type='sweat'
        )
    else:
...
def _bomb_init(
    ...
    if self.bm_type == 'ice_bubble':
        ...
        self.node.mesh = None
        self.shield_ice = bs.newnode(
            'shield', owner=self.node, attrs={'color': (0.5, 1.0, 7.0), 'radius': 0.6}
        )
...
def bomb_handlemessage(self, msg: Any) -> Any:
    ...
    if isinstance(msg, bs.DieMessage):
        ...
        if self.blast_type in ('ice', 'ice_bubble'):
            bomb.BombFactory.get().freeze_sound.play(10)
...
def powerup_translated(self, type: str):
    powerups_names = {
        'triple_bombs': babase.Lstr(resource='helpWindow.' + 'powerupBombNameText'),
        'ice_bombs': babase.Lstr(resource='helpWindow.' + 'powerupIceBombsNameText'),
        'punch': babase.Lstr(resource='helpWindow.' + 'powerupPunchNameText'),
        'impact_bombs': babase.Lstr(resource='helpWindow.' + 'powerupImpactBombsNameText'),
        'land_mines': babase.Lstr(resource='helpWindow.' + 'powerupLandMinesNameText'),
        'sticky_bombs': babase.Lstr(resource='helpWindow.' + 'powerupStickyBombsNameText'),
        'shield': babase.Lstr(resource='helpWindow.' + 'powerupShieldNameText'),
        'health': babase.Lstr(resource='helpWindow.' + 'powerupHealthNameText'),
        'curse': babase.Lstr(resource='helpWindow.' + 'powerupCurseNameText'),
...
def _pbx_(
    ...
    def util_text(
        ...
        scale: float = 1,
        color: list = [1, 1, 1],
        position: list = [0, 0.7, 0],
        colors_name: bool = False,
    ):
        m = bs.newnode(
            'math',
            owner=self.node,
            attrs={'input1': (position[0], position[1], position[2]), 'operation': 'add'},
        )
        self.node.connectattr('position', m, 'input2')
        self.texts[type] = bs.newnode(
            'text',
            owner=self.node,
            attrs={
                'text': str(text),
        ...
        bs.animate(self.texts[type], 'scale', {0: 0.017, 0.4: 0.017, 0.5: 0.01 * scale})
    ...
    def update_time(time):
        if hasattr(self, 'texts') and 'Time' in self.texts and self.texts['Time']:
    ...
    if config['Powerup Time']:
        ...
        util_text(
            'Time', time2, scale=1.5, color=(2, 2, 2), position=[0, 0.9, 0], colors_name=False
        )
    ...
    if config['Powerup With Shield']:
        ...
        self.shield = bs.newnode(
            'shield', owner=self.node, attrs={'color': (1, 1, 0), 'radius': 1.3 * scale}
        )
    ...
    if config['Powerup Name']:
        util_text('Name', self.poweruptype, scale=1.2, position=[0, 0.4, 0], colors_name=True)
...
def _init_spaz_(self, *args, **kwargs):
    ...
    self.tankshield = {"Tank": False, "Reduction": False, "Shield": None}
...
def spaz_on_punch_press(self) -> None:
    ...
    if self.tankshield['Tank']:
        try:
            ...
            shield = bs.newnode(
                'shield', owner=self.node, attrs={'color': (4, 1, 4), 'radius': 1.3}
            )
...
def new_handlemessage(self, msg: Any) -> Any:
    ...
    if isinstance(msg, bs.PickedUpMessage):
        ...
        bs.timer(0.001, bs.WeakCallPartial(self.shatter))

    elif isinstance(msg, bs.ImpactDamageMessage):
        bs.timer(0.001, bs.WeakCallPartial(self._hit_self, msg.intensity))
        ...
        if msg.poweruptype == 'triple_bombs':
            ...
            if self.powerups_expire:
                ...
                self._multi_bomb_wear_off_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME, babase.CallPartial(self._multi_bomb_wear_off)
                )
            ...
            if self.powerups_expire:
                ...
                self._bomb_wear_off_flash_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME - 2000, babase.CallPartial(self._bomb_wear_off_flash)
                )
                self._bomb_wear_off_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME, babase.CallPartial(self._bomb_wear_off)
                )
            ...
            if self.powerups_expire:
                ...
                self._boxing_gloves_wear_off_flash_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME - 2000, bs.WeakCallPartial(self._gloves_wear_off_flash)
                )
                self._boxing_gloves_wear_off_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME,
                    bs.WeakCallPartial(self._gloves_wear_off),
                )
            ...
            if self.powerups_expire:
                ...
                self._bomb_wear_off_flash_timer = bs.Timer(
                    ...
                    bs.WeakCallPartial(self._bomb_wear_off_flash),
                )

                self._bomb_wear_off_timer = bs.Timer(
                    POWERUP_WEAR_OFF_TIME / 1000.0, bs.WeakCallStrict(self._bomb_wear_off)
                )
                ...
                for attr in ['materials', 'roller_materials']:
                    ...
                    if factory.curse_material in materials:
                        setattr(
                            self.node,
                            attr,
                            tuple(m for m in materials if m != factory.curse_material),
                        )
            ...
            if self.powerups_expire:
                ...
                self.ice_man_flash_timer = bs.Timer(
                    (ice_man_time - 2000) / 1000.0, babase.CallPartial(_ice_man_off_flash, self)
                )

                self.ice_man_timer = bs.Timer(
                    ice_man_time / 1000.0, babase.CallPartial(_ice_man_wear_off, self)
                )
            ...
            if self.powerups_expire:
                ...
                self.speed_flash_timer = bs.Timer(
                    (speed_time - 2000) / 1000.0, babase.Call(_speed_off_flash, self)
                )

                self.speed_timer = bs.Timer(
                    speed_time / 1000.0, bs.WeakCallPartial(_speed_wear_off, self)
                )
        ...
        if self.node.invincible:
            SpazFactory.get().block_sound.play(1.0, self.node.position)
            return None
        if self.shield:
            return None
        if not self.frozen:
            ...
            bs.timer(5.0, bs.WeakCallPartial(self.handlemessage, bs.ThawMessage()))
            if self.hitpoints <= 0:
                self.shatter()
        ...
        def fire(time, damage):
            if not self.shield and not self._dead:
                ...
                bs.show_damage_count(f'-{damage}HP', self.node.position, msg.force_direction)
        ...
        if msg.hit_subtype == 'fly':
            ...
            if self.shield:
                ...
                if self.shield_hitpoints < 0:
                    self.shield.delete()
                    self.shield = None
                    SpazFactory.get().shield_down_sound.play(1.0, self.node.position)
            ...
            if self.shield:
                ...
                bs.show_damage_count(
                    (f'-{int(hitpoints / 10)}%'), self.node.position, msg.force_direction
                )
        ...
        if self.shield:
            ...
            self.shield.hurt = 1.0 - float(self.shield_hitpoints) / self.shield_hitpoints_max
            ...
            if self.shield_hitpoints <= 0:
                ...
                SpazFactory.get().shield_hit_sound.play(0.5, self.node.position)
        ...
        if self.tankshield['Reduction']:
            ...
            bs.show_damage_count('-' + str(int(damage / 10)) + '%', msg.pos, msg.force_direction)
        ...
        if self.edg_eff:
            ...
            PopupText(
                text=f'+{int(dmg_dism / 10)}%',
                scale=1.5,
                position=self.node.position,
                color=(0, 1, 0),
            ).autoretain()
            bs.animate_array(
                self.node, 'color', 3, {0: (0, 1, 0), 0.39: (0, 2, 0), 0.4: self.color[0]}
            )
        ...
        if msg.hit_type == 'punch':
            ...
            if damage > 350:
                assert msg.force_direction is not None
                bs.show_damage_count(
                    '-' + str(int(damage / 10)) + '%', msg.pos, msg.force_direction
                )

            if msg.hit_subtype == 'super_punch':
                SpazFactory.get().punch_sound_stronger.play(1.0, self.node.position)
            if damage > 500:
                sounds = SpazFactory.get().punch_sound_strong
                sound = sounds[random.randrange(len(sounds))]
            else:
                sound = SpazFactory.get().punch_sound
            sound.play(1.0, self.node.position)
            ...
            flash = bs.newnode(
                'flash',
                attrs={'position': punchpos, 'size': 0.17 + 0.17 * hurtiness, 'color': flash_color},
            )
        ...
        if self.hitpoints > 0:
            ...
            if self._cursed and damage > 0:
                bs.timer(
                    0.05, bs.WeakCallPartial(self.curse_explode, msg.get_source_player(bs.Player))
                )
        ...
        self._last_stand_pos = (msg.position[0], msg.position[1], msg.position[2])
        if self.node:
            self.node.handlemessage(
                'stand', msg.position[0], msg.position[1], msg.position[2], msg.angle
            )
        ...
        if node and (node not in self._punched_nodes):
            ...
            punch_momentum_angular = self.node.punch_momentum_angular * self._punch_power_scale
...
class PowerupManagerWindow(PopupWindow):
    def __init__(self, transition='in_right'):
        ...
        self._backButton = b = bui.buttonwidget(
            parent=self._root_widget,
            autoselect=True,
            position=(60, self._height - 15),
            size=(130, 60),
            scale=0.8,
            text_scale=1.2,
        ...
        for tab in range(self.count):
            for tab2 in range(columns):
                ...
                self.tab_buttons[tag] = bui.buttonwidget(
                    ...
                    enable_sound=False,
                    button_type='square',
                    on_activate_call=babase.CallPartial(self._set_tab, tag, sound=True),
                )
    ...
    def _set_tab(self, tab, sound: bool = False):
        ...
        self._scrollwidget = bui.scrollwidget(
            parent=self._root_widget,
            position=(self._width * 0.08, 51 * 1.8),
        ...
        if tab == 'Action 4':
            ...
            self._scrollwidget = bui.scrollwidget(
                ...
                size=(self._sub_width - 140, self._scroll_height + 60 * 1.2),
                capture_arrows=True,
                center_small_content=False,
                selection_loops_to_parent=True,
                claims_left_right=True,
                claims_up_down=False,
                color=(0.3, 0.3, 0.4),
            )
            bui.textwidget(edit=self.titletext, text=babase.Lstr(resource='storeText'))

        elif tab == 'About':
            bui.textwidget(edit=self.titletext, text=babase.Lstr(resource='gatherWindow.aboutText'))
        else:
        ...
        if tab == 'Action 1':
            ...
            self.button_cls_power = bui.buttonwidget(
                parent=self._root_widget,
                position=(500, self._width * 0.61),
                size=(50, 50),
                autoselect=True,
                scale=1,
                label=('%'),
            ...
            for (
                cls
            ) in self.list_cls_power:  # this is very important so that pupups don't accumulate
                if cls not in rewindow:
                    cls.delete()

        elif tab == 'Action 4':
            self.button_coin = bui.buttonwidget(
                parent=self._root_widget,
                icon=bui.gettexture('coin'),
                position=(550, self._width * 0.614),
                size=(160, 40),
                textcolor=(0, 1, 0),
                color=(0, 1, 6),
                scale=1,
        ...
        if tab == 'Action 1':
            ...
            for power in self.default_power_list:
                ...
                t = bui.textwidget(
                    parent=c,
                    position=(position[0] - 30, position[1] - 15),
                    size=(width, 50),
                    h_align="center",
                    color=(bui.app.ui_v1.title_color),
                    text=label,
                ...
                for direc in ['-', '+']:
                    bui.buttonwidget(
                        parent=c,
                        autoselect=True,
                        position=(position[0] + 270 + dipos, position[1] - 10),
                        size=(100, 100),
                        scale=0.4,
                        label=direc,
                        button_type='square',
                        text_scale=4,
                        on_activate_call=babase.CallPartial(self.apperance_powerups, power, direc),
                    )
                ...
                textwidget = bui.textwidget(
                    parent=c,
                    position=(position[0] + 190, position[1] - 15),
                    size=(width, 50),
                    h_align="center",
            ...
            popup = PopupMenu(
                ...
                on_value_change_call=babase.CallPartial(self._all_popup, 'Powerup Style'),
            )
            ...
            for direc in ['-', '+']:
                bui.buttonwidget(
                    parent=c,
                    autoselect=True,
                    position=(position[0] + 310 + dipos, position[1] - 100),
                    size=(100, 100),
                    repeat=True,
                    scale=0.4,
                    label=direc,
                    button_type='square',
                    text_scale=4,
                    on_activate_call=babase.CallPartial(self._powerups_scale, direc),
                )
                dipos += 100

            txt_scale = config['Powerup Scale']
            self.txt_scale = bui.textwidget(
                parent=c,
                position=(position[0] + 230, position[1] - 105),
                size=(width, 50),
                scale=1.1,
                h_align="center",
                color=(0, 1, 0),
            ...
            t = bui.textwidget(
                parent=c,
                position=(position[0] - 60 + wt, position[1] - 100),
                size=(width, 50),
                maxwidth=width * 0.9,
                scale=1.1,
                h_align="center",
            ...
            self.check = bui.checkboxwidget(
                parent=c,
                position=(position[0] + 30, position[1] - 230),
                value=config['Powerup Name'],
                on_value_change_call=babase.CallPartial(self._switches, 'Powerup Name'),
            ...
            self.check = bui.checkboxwidget(
                parent=c,
                position=(position[0] + 30, position[1] - 230 * 1.3),
                value=config['Powerup With Shield'],
                on_value_change_call=babase.CallPartial(self._switches, 'Powerup With Shield'),
                maxwidth=self._scroll_width * 0.9,
                text=getlanguage('Powerup With Shield'),
                autoselect=True,
            )
            ...
            if STORE['Buy Option']:
                self.check = bui.checkboxwidget(
                    parent=c,
                    position=(position[0] + 30, position[1] - 230 * 1.6),
                    value=config['Powerup Time'],
                    on_value_change_call=babase.CallPartial(self._switches, 'Powerup Time'),
                    maxwidth=self._scroll_width * 0.9,
                    text=getlanguage('Powerup Time'),
                    autoselect=True,
                )
            ...
            if not STORE['Buy Percentage']:
                ...
                t = bui.textwidget(
                    parent=c,
                    position=(position[0] - 14, position[1] + 70),
                    size=(30 + width, 50),
                    h_align="center",
                ...
                b = bui.buttonwidget(
                    ...
                    repeat=True,
                    scale=0.6,
                    label=self.charstr[3],
                    button_type='square',
                    text_scale=2,
                    on_activate_call=babase.CallPartial(self.tank_shield_percentage, 'Decrement'),
                )
                ...
                b = bui.buttonwidget(
                    ...
                    size=(100, 100),
                    scale=0.6,
                    label=self.charstr[2],
                    button_type='square',
                    on_activate_call=babase.CallPartial(self.tank_shield_percentage, 'Increment'),
                )
                ...
                self.tank_text = bui.textwidget(
                    parent=c,
                    position=(position[0] - 14, position[1] + 5),
                    size=(30 + width, 50),
                    h_align="center",
                    text=str(porcentaje) + '%',
                    color=color,
                ...
                b = bui.buttonwidget(
                    ...
                    button_type='square',
                    text_scale=2,
                    on_activate_call=babase.CallPartial(self.health_damage_percentage, 'Decrement'),
                )
                ...
                b = bui.buttonwidget(
                    ...
                    label=self.charstr[2],
                    button_type='square',
                    on_activate_call=babase.CallPartial(self.health_damage_percentage, 'Increment'),
                )
            ...
            for power in self.default_power_list:
                ...
                t = bui.textwidget(
                    parent=c,
                    position=(position[0] + 170, position[1] - 10),
                    size=(width, 50),
                    h_align="center",
                    color=(0, 1, 0),
                    text=(f'{ptg}%'),
            ...
            if not self._scrollwidget or not self._scrollwidget.exists():
                return
            self._tab_container = c = bui.containerwidget(
                parent=self._scrollwidget,
            ...
            for store in store_items():
                ...
                b = bui.buttonwidget(
                    parent=c,
                    autoselect=True,
                    position=(position[0] + 210 - n_pos, position[1]),
                    size=(250, 80),
                    scale=0.7,
                    label=text,
                    ...
                    color=color,
                    iconscale=1.7,
                    on_activate_call=babase.CallPartial(self._buy_object, store, p),
                )
                ...
                b = bui.buttonwidget(
                    parent=c,
                    autoselect=True,
                    position=(position[0] + 210 - n_pos, position[1] + 55),
                    size=(s, s + 30),
                    scale=1,
                    label='',
                    color=color,
                ...
                i = bui.imagewidget(
                    ...
                    size=(s, s),
                    texture=bui.gettexture(preview),
                )

                t = bui.textwidget(
                    parent=c,
                    position=(position[0] + 270 - n_pos, position[1] + 101),
                    h_align="center",
                    color=(bui.app.ui_v1.title_color),
                    text=label,
                    v_align="center",
            ...
            t = bui.textwidget(
                ...
                h_align="center",
                color=(bui.app.ui_v1.title_color),
                text=babase.Lstr(resource='settingsWindowAdvanced.enterPromoCodeText'),
                v_align="center",
                maxwidth=width * 1.3,
            )

            self.promocode_text = bui.textwidget(
                parent=c,
                position=(position[0] + 80, position[1] - 100),
                size=(width + 60, 50),
                scale=1,
                editable=True,
                h_align="center",
                ...
                maxwidth=width * 1.3,
                max_chars=30,
                description=babase.Lstr(resource='settingsWindowAdvanced.enterPromoCodeText'),
            )

            self.promocode_button = bui.buttonwidget(
                parent=c,
                position=(position[0] + 160, position[1] - 170),
                size=(200, 60),
                scale=1.0,
    ...
    def tank_shield_percentage(self, tag):
        ...
        if porcentaje > 59:
            ...
            color = (0, 1, 0.8)
        bui.textwidget(edit=self.tank_text, text=str(porcentaje) + '%', color=color)
    ...
    def health_damage_percentage(self, tag):
        ...
        if porcentaje > 59:
            ...
            color = (0, 1, 0.8)
        bui.textwidget(edit=self.hlg_text, text=str(porcentaje) + '%', color=color)
    ...
    def apperance_powerups(self, powerup: str, ID: str):
        ...
        bui.textwidget(
            edit=self.listpower[powerup], text=str(powerups[powerup]), color=cls_pow_color()[enum]
        )
    ...
    def _powerups_scale(self, ID: str):
        ...
        bui.textwidget(edit=self.txt_scale, text=str(config['Powerup Scale']))
    ...
    def _buy_object(self, tag: str, price: int):
        store = BearStore(
            value=tag, price=price, callback=babase.CallPartial(self.store_refresh, tag)
        )
        store.buy()
