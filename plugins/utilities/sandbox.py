...
def error(real): # not fake
    ...
    with ga().context: img = bs.gettexture("ouyaAButton")
    push(real, color=(1, 0, 0), top=Nice.top_msg, image=img)

def ding(fake): # fake
    if Nice.do_ding: bui.getsound('ding').play()
    with ga().context: img = bs.gettexture("ouyaOButton")
    push(fake, color=(0, 1, 0), top=Nice.top_msg, image=img)

def var(s, v=None):
    cfg = bui.app.config
    if v is None:
        try: return cfg['sb_'+s]
        except: return 0
    else: cfg['sb_'+s] = v; cfg.commit()
...
class Nice(mm.MainMenuWindow):
    ...
    a = var('do_ding'); do_ding = a if isinstance(a, bool) else True
    a = var('while_control'); while_control = a if isinstance(a, bool) else True
    ...
    def pause(s, b):
        try: ga().globalsnode.paused = b
        except AttributeError: pass # i am not the host
    def __init__(s, call_sand=True):
        ...
        if Nice.pause_when_bots:
            with ga().context: bs.timer(0, bs.Call(s.pause, True))
        if str(ga()) != old_ga: s.on_ga_change()
        if call_sand:
    ...
    def center(s):
        if s.soff == (0, 0): error("Sandbox is already centered what are u doing"); return
        s.soff = (0, 0)
    ...
    def kill(s, wosh=False, who=None, keep_hl=False, anim=True, rev=False):
        try: bui.containerwidget(edit=who, transition=(s._anim_out if not rev else s._anim_outv) if anim else None)
        except: pass
        if wosh: bui.getsound('swish').play()
        if not keep_hl: s.hl3(None, False)

    def lol_window(s):
        if ga() is None: push("no MORE for you bud,\nyou are not the host.", color=(1, 1, 0)); return
    ...
    def load_window(s):
        if Nice.pause_when_bots: error("Cannot use Load Window while game is paused"); return
        s.load_dux = None
    ...
    def do_load(s):
        i = s.load_dux
        if i is None: error("Select a preset or get out."); return
        s.load_preset(i)
    ...
    def collect_dim(s, x, y, z, outer):
        ...
        if emp: error(f"{emp} value cannot be empty!"); return
        try: v = eval(f"({t1}, {t2}, {t3})")
        except Exception as e: error(str(e)); return
        if outer: n.vignette_outer = v
        else: n.vignette_inner = v
    ...
    def light_window(s):
        ...
        global light_pick
        tent = ga().globalsnode.tint; ntent = s.negate(tent)
    ...
    def LTWN_load_teams(s):
        for w in s.LTWN_sub.get_children(): w.delete()
    ...
    def do_nuke(s):
        if len(Nice.lmao_teams) < 1: error("Remove what u blind?"); return
        if s.team_to_nuke is None: error("Select a team"); return
        t = s.team_to_nuke
        s.team_to_nuke = None
        ga().remove_team(t)
    ...
    def lol_teams_window_done(s):
        ...
        if not LTN: error("Enter a team name or just leave a space there"); return
    ...
    def tweak_window(s):
        if ga() is None: push("You don't meet the minimum requirements\nto use BorddTweaker: BEING THE HOST.", color=(1, 1, 0)); return
        p = s.brobordd.node.position
        with ga().context: nuds = bs.getnodes()
        s.nodes = []
        s.nodes_2d = []
        ...
        for i in nuds:
            ...
            try: obj = i.getdelegate(object)
            except: obj = None
            if len(pos) == 3: s.nodes.append([]); s.nodes[-1].append(pos); s.nodes[-1].append(i), s.nodes[-1].append(obj)
            elif len(pos) == 2: s.nodes_2d.append([]); s.nodes_2d[-1].append(pos); s.nodes_2d[-1].append(i); s.nodes_2d[-1].append(obj);
        s.nodes.append([]); s.nodes[-1].append((0, 0, 0)); s.nodes[-1].append(ga().globalsnode), s.nodes[-1].append(None)
        ...
        for n in range(len(s.nodes)):
            ...
            except: s.tweak_name.append(f"idk {n}"+obj)
            try:
                t = str(s.nodes[n][1].color_texture)
                on = t.find('"'); off = t.find('"',on+1)
                ...
                try:
                    ...
                    try: s.tweak_texture.append(str(s.nodes[n][1].text))
                    except: s.tweak_texture.append(bui.gettexture("tv"))
            try: thing = s.what_is(s.nodes[n][1].mesh)
            except: continue
        ...
        for n in range(len(s.nodes_2d)):
            ...
            except: s.tweak_name_2d.append(f"idk {n}"+obj)
            try:
                ...
                try:
                    ...
                    try: s.tweak_texture_2d.append(str(s.nodes_2d[n][1].text))
                    except: s.tweak_texture_2d.append(bui.gettexture("tv"))
            try: thing = s.what_is(s.nodes_2d[n][1].mesh)
            except: continue
    ...
    def load_tweak_nodes(s, t=True):
        ...
        # clean up
        for c in s.tweak_sub.get_children(): c.delete()
...
#        except Exception as e: error(str(e))
        ...
        if isinstance(k, str):
            bui.textwidget(edit=s.tweak_preview_text2, text=tt[s.tweak_dux], color=s.get_type_color("str"))
            bui.buttonwidget(edit=s.tweak_preview_image, texture=bui.gettexture("tv"), color=(1, 1, 1))
        else: bui.buttonwidget(edit=s.tweak_preview_image, texture=tt[s.tweak_dux], color=(1, 1, 1))
    ...
    def draw_locator(s, pos, pos2=(0, 0)):
        try:
            for node in s.royna: node.delete()
        except: pass
        s.royna = []
    ...
    def do_tweak(s):
        b = s.tweak_2d
        i = s.tweak_dux
        try: node = s.nodes_2d[i] if b else s.nodes[i] # list of 3
        except TypeError: error("Select a node dum dum"); return
    ...
    def load_tweakz_nodes(s, node, t=True):
        ...
        for i in dir(blex):
            tn.append(i)
            try: attr = getattr(blex, i) if i not in ["punch_position", "punch_velocity", "punch_momentum_linear"] else (0, 0, 0) # gay
            except: attr = None
            typez.append(str(type(attr).__name__))
            value.append(attr)
        if alex[2] is None and not t: tn.append("No object lol")
        s.tweakz_name = tn
        s.tweakz_type = typez
        s.tweakz_value = value
        # clean up
        for c in s.tweakz_sub.get_children(): c.delete()
    ...
    def get_type_color(s, t):
        ...
        if t == "str": c = (0, 0.6, 0) # green
        elif t == "float": c = (0, 1, 1) # cyan
        elif t == "tuple": c = (1, 0.6, 1) # light pink
        elif t == "bool": c = (1, 1, 0) # yellow
        elif t == "NoneType": c = (0.4, 0.4, 0.4) # grey
        elif t == "Texture": c = (0.6, 0, 0.8) # purple
        return c
    ...
    def tweak_this(s):
        i = s.tweakz_dux
        mode = 0
        try: name = s.tweakz_current_name.split(" ")[0]+"."+s.tweakz_name[i]
        except TypeError: error("Select an attribute bruh"); return
        try: value = s.tweakz_value[i]
        except IndexError: error("Tweak no object? are you high?"); return
        typ = s.tweakz_type[i]
        if typ == "NoneType": error("Can't modify NoneType,\nidk what type should it be."); return
        if str(value).startswith("<"):
            if not typ == "Texture": error(f"{name} is not tweakable!"); return
            mode = 1 # texture picker
        if typ == "tuple" and str(value).startswith("(<"): error("This tuple is not tweakable!"); return
    ...
    def gather_tweakz(s, name, typ, mode):
        ...
        if not value: error("If you won't enter something THEN U CAN PRESS CANCEL.")
        # my nice yet stupid validator
        elif typ == "bool" and value.lower() in ["true", "false"]: v = value.lower() == 'true'
        elif typ == "bool": error("bool must be True or False, not '{}'".format(value)); return
        elif typ == "float":
            try: v = float(value)
            except: error("float must be a number (decimal), not '{}'".format(value)); return
        elif typ == "int":
            try: v = int(value)
            except: error("int must be a number (no decimals), not '{}'".format(value)); return
        elif typ == "tuple":
            try: e = eval(value); v = e if type(e) is tuple else bro
            except: error(f"tuple must be a goddamn tuple, not '{value}'\nlike this: (1.23, 1.91, 0.69)"); return
        elif typ == "str": v = value # string anything u like
        elif typ == "Texture":
            if value not in all_texture: error(f"Unknown texture '{value}',\nuse 'white', 'black' or 'null' for empty ones"); return
            with ga().context: v = bs.gettexture(value)
        # apply value to node
        try:
            with ga().context: setattr(node[1 if s.tweakz_is_node else 2], name, v)
        except Exception as e: error(str(e) if str(e).strip() else f"No error info, repr(e):\n{repr(e)}")
        else: ding(f"Tweaked!")
        s.kill(True, s.TTW)

    def call_this(s):
        i = s.tweakz_dux
        try: name = s.tweakz_name[s.tweakz_dux]
        except TypeError: error("You better call a doctor instead,\nno attribute is selected"); return
        try: attr = getattr(s.tweakz_current_node[1 if s.tweakz_is_node else 2], name)
        except AttributeError as e: error("Node no longer exists\nwhat are you doing here?" if "No object lol" not in str(e) else "Sure, equip a brain first"); return
    ...
    def do_call_this(s, attr, name):
        ...
        if t != "":
            args = t.split(",")
            try: args = [eval(a.strip()) for a in args]
            except Exception as e: error(str(e)); return
        else: args = []
        try:
            with ga().context: out = attr(*args)
            ding(f"Success! calling '{name}' (dumped to terminal)\nwith arguments {args}\noutputted: {out}")
            s.kill(True, s.CTW)
        except Exception as e: error(str(e) if str(e).strip() else f"No error info, repr(e):\n{repr(e)}")
        else: print(f"SandBox.ByBordd: calling '{name}' outputted:\n{out}")

    def drop_window(s):
        if ga() is None: push("Drop? looks like you dropped your brain somewhere,\nyou are not the host.", color=(1, 1, 0)); return
    ...
    def collect_drop_attrs(s):
        ...
        try: v1 = float(t1)
        except: error(f"Invalid gravity_scale value '{t1}'\nrequired value: float\nexample: 6.89"); return
        try: v2 = float(t2)
        except: error(f"Invalid reflection_scale value '{t2}'\nrequired value: float\nexample: 6.89"); return
    ...
    def check_drop_attrs(s, i, b):
        if not i: Nice.node_sticky = b
        if i == 1: Nice.node_reflect = b; bui.checkboxwidget(edit=s.drop_radio2, value = False); Nice.node_reflect2 = False
        if i == 2: Nice.node_reflect2 = b; bui.checkboxwidget(edit=s.drop_radio1, value = False); Nice.node_reflect = False

    def do_drop(s):
        p = Nice.drop_cords
        if p[0] == 69123: error("No position provided"); return
        i = Nice.drop_indox
        ...
        with ga().context:
            ...
            if powerup:
                ...
                bss = 1.0; br = 2.0
                if "big_bomb" in Nice.pending2: bss = 2.5; br = 12.5
                if i == 9: # TNT
                    n = Bomb(position=p, bomb_type='tnt', bomb_scale=bss, blast_radius=br).autoretain()
                    n = n.node
                if i == 10: # Peaceful Mine
                    n = Bomb(position=p, bomb_type='land_mine', bomb_scale=bss, blast_radius=br).autoretain()
                    n = n.node
                if i == 11: # Lit Mine
                    n = Bomb(position=p, bomb_type='land_mine', bomb_scale=bss, blast_radius=br).autoretain().arm()
                    n = n.node
            ...
            if Nice.node_reflect: n.reflection = 'powerup'
            elif Nice.node_reflect2: n.reflection = 'soft'
            n.reflection_scale = Nice.node_reflection_scale
            if "long_fuse" in Nice.pending2: n.fuse_length = 5
    ...
    def custom_drop_window(s):
        ...
        def collect(s):
            w = x; a=[]
            for i in range(3):
                try: a.append(float(cast(str, bui.textwidget(query=w))))
                except:
    ...
    def spawn_window(s):
        if ga() is None: push('Spawning requires you to be the host!', color=(1, 1, 0)); return
        ...
        bui.buttonwidget(edit=nice_view, tint_texture=bui.gettexture(bot_texture[bot_name.index(Nice.val_attrs[1])]+'IconColorMask'))
        bui.buttonwidget(edit=nice_view, tint_color=Nice.val_attrs[6], tint2_color=Nice.val_attrs[11])
        ...
        try:
            if cords[0] != 69123: s.update_cords_view()
        except TypeError: error('No coordinates set')
    ...
    def Button(s):
        ...
        def closeBox(self):
            if self.sbox.exists(): bui.buttonwidget(edit=self.sbox, icon=bui.gettexture('chestIcon'))
    ...
    def custom_window(s):
        ...
        global cords
        try: txt = str(cords[0]) if cords[0] != 69123 else "0"
        except TypeError:
            cords = (0, 0, 0)
            txt = "0"
    ...
    def gather(s, ran=False, close=True):
        ...
        for i in range(len(ins)):
                ...
                if t == 'float':
                    ...
                    if not v in bot_name and i == 1:
                        if ran:
                            ...
                            if v in w_bot_name:
                                push(f"Did you mean '{bot_name[w_bot_name.index(v)]}'?", color=(0, 0.6, 1))
                            return
                    elif i == 1:
                        if ran: v = random.choice(bot_name)
                        try: s.spawn(bot_name.index(v)) # update preview
                        except TypeError: Nice.spawn(Nice, bot_name.index(v))
                        bui.textwidget(edit=ins[i], text=str(v))
                        ...
                        if ran:
                            ...
                            if v in w_bomb_type:
                                push(f"Did you mean '{bomb_type[w_bomb_type.index(v)]}'?", color=(0, 0.6, 1))
                            return
            ...
            if bui.Widget.get_widget_type(ins[i]) == 'checkbox' and ran:
                ...
                a = []
                for r in range(3): a.append(random.uniform(0.0, 1.0))
                a = (float(a[0]), float(a[1]), float(a[2]))
                bui.buttonwidget(edit=ins[i], label=f"{str(a[0]+0.01)[:3]} {str(a[1]+0.01)[:3]}, {str(a[2]+0.01)[:3]}", color=(a[0], a[1], a[2]), textcolor=(1-a[0], 1-a[1], 1-a[2]))
                Nice.val_attrs[i] = a
                bui.buttonwidget(edit=nice_view, tint_texture=bui.gettexture(bot_texture[bot_name.index(Nice.val_attrs[1])]+'IconColorMask'))
                bui.buttonwidget(edit=nice_view, tint_color=Nice.val_attrs[6], tint2_color=Nice.val_attrs[11])
        if not ran and close: s.kill(True, attr_widget, rev=True)
        elif ran:
            bui.getsound('cashRegister2').play()
            if s.auto_spawn_on_random: s.do_spawn()

    def control_window(s):
        if ga() is None: push('How control and you are not the host?', color=(1, 1, 0)); return
        ...
        try: a = bomb_control
        except NameError: bomb_control = False
        fresh = True
        ...
        try: test = currently_txt
        except NameError: test = 'Control started\nnow tap a bot'
        ...
        for i in range(len(lmao)):
            try: alive = lmao_bots[i].node.hurt < 1
            except IndexError:
                s.kill(True, control_widget)
                push('Wait for that bot to spawn first')
                return
            except AttributeError: alive = False
    ...
    def preview(s, i, alive, mod=0):
        ...
        if i == 69123:
            ...
            bui.buttonwidget(edit=preview_image, texture=bui.gettexture("achievementSharingIsCaring"), tint_texture=None, mask_texture=None)
            s.select_all_bots = True
            return
        s.select_all_bots = False
        try: bui.checkboxwidget(edit=s.select_all, value=False)
        except: pass
        drux = i
        ...
        bui.textwidget(edit=preview_text, text=_lmao[i], color=((1, 1, 1) if alive else (0.6, 0.6, 0.6)))
        bui.textwidget(edit=preview_text2, text=_lmao_chars[i], color=((0.8, 0.8, 0.8) if alive else (0.4, 0.4, 0.4)))
        bui.buttonwidget(edit=preview_image, tint_texture=bui.gettexture(bot_texture[bot_name.index(_lmao_chars[i])]+'IconColorMask'))
        if mod != 3: bui.buttonwidget(edit=preview_image, tint_color=val_arr[drux][6], tint2_color=val_arr[drux][11])
        if alive:
            try:
                if not on_control: s.hl3(i)
            except AttributeError:
                error('this bot is dead, reopen window')
                alive = False
                #TODO array containing lmao bot text ins so we can live change
        else: s.hl3(None, False)
        try: hurt = _lmao_bots[i].node.hurt if mod != 3 else _lmao_bots[i].actor.node.hurt
        except AttributeError: pass # bot is GONE
        try: hp_txt = f'HP: {"%.2f" % (100 * (1 - hurt))}%'
        except AttributeError: hp_txt = 'HP: 0.00%'
        except NameError: hp_txt = 'HP: 0.00% (gone)'
        bui.textwidget(edit=preview_text3, text=hp_txt, color=((0.8, 0.8, 0.8) if alive else (0.4, 0.4, 0.4)))
        dux = i
        if not mod: currently_dux = i
        elif mod == 2: effect_dux = i
        elif mod == 3: effect_dux2 = i
        bot = _lmao_bots[dux]
        char = _lmao_chars[dux]
        skin = bot_texture[bot_name.index(char)] # neoSpaz
        icon = bui.gettexture(skin+'Icon') # texture: neoSpazIcon
        bui.buttonwidget(edit=preview_image, texture=icon, color=((1, 1, 1) if alive else (0.6, 0.6, 0.6)))
        if mod or on_control: s.assign()

    def start_or_stop(s):
        global on_control, start_stop, lmao, fresh, currently_dux, lmao_bots
        try: KO = lmao_bots[currently_dux].node.hurt == 1
        except NameError: error("Start your brain first"); return
        except AttributeError: KO = True
        if KO:
        ...
        bui.buttonwidget(edit=start_stop, icon=bui.gettexture('ouyaAButton' if b else 'ouyaOButton'))
        if b:
            if random.choice([1,0,0]): push('You can switch control by selecting another bot')
            s.reset_bright_bots()
        KO = False
        s.assign()
    ...
    def assign(s): #bool):
        ...
        try:
            if control_widget.exists(): allow_assign = True
            if not allow_assign: return
            allow_assign = False # for outside control
        except NameError: return # coming from modify widget lol
        for i in ga().players:
            if i.sessionplayer.inputdevice.client_id == -1:
                i.resetinput() # clean up previous control
                with ga().context: i.actor.connect_controls_to_player()
                if not on_control:
                    push('Stopped control for good', color=(0.4, 0.1, 0.2))
                    try: s.draw()
                    except: s.draw(s)
                    old_dux = None
                    try: bui.textwidget(edit=preview_text4, text="Press start\nto start controlling")
                    except NameError: pass # modify again
                    except RuntimeError: pass # bot died, outside UI control
                ...
                try:
                    ...
                    on_control = False
                    s.assign()
                    bui.buttonwidget(edit=start_stop, label='Start', icon=bui.gettexture("ouyaOButton"))
                    return
                if cast(str, bui.textwidget(query=preview_text4)) == 'Character':
                    push('good, now select a bot to control', color=(0, 0.5, 0))
                    return
                if Nice.while_control: i.actor.node.invincible = True
                try:
    ...
    def config_window(s):
        if ga() is None: push('Sure, ask the HOST that is obv not YOU', color=(1, 1, 0)); return
...
#        s.do_your_thing(ga().globalsnode.slow_motion, False)
    ...
    def conf(s, i, b):
        if not i: Nice.while_control = b; var('while_control', b)
        elif i == 1: Nice.notify_bot_ded = b; var('notify_bot_ded', b)
        elif i == 2:
            Nice.pause_when_bots = b; var('pause_when_bots', b)
            s.pause(b)
            ding('Applied now!')
        elif i == 3: Nice.top_msg = b; var('top_msg', b)
        elif i == 4: Nice.lite_mode = b; var('lite_mode', b)
        elif i == 5: Nice.animate_camera = b; var('animate_camera', b)
        elif i == 6: Nice.do_ding = b; var('do_ding', b)
    ...
    def do_your_thing(s, b):
        ...
        epic = "Epic " if b else ""
        def fade(node, i):
            try: t = title_node[i].text = f"{epic}{title[i]}"
            except: pass
        ...
        with ga().context:
            if virgin:
                ...
                for n in bs.getnodes()[::-1]:
                    if hasattr(n, 'text'):
                        if 'ARG' in n.text: continue
                        if 'gameNames' not in n.text: continue
                        try:
                            try: title.append(json.loads(n.text)['s'][0][1]['t'][1])
                            except:
                                try:
                                    title.append(json.loads(n.text)['t'][1])
                                except: continue
                            title_node.append(n)
                        except: pass # i swear it cusses about int and stuff i had to shut it up
            for i in range(len(title_node)):
                if not title_node[i].exists(): continue
                try: bs.animate(title_node[i], 'opacity', {0.0: 1.0, 0.1: 0.0})
                except: return # what are we doing here
                bs.timer(0.08, bs.Call(fade, title_node[i], i))

    def mod_window(s):
        if ga() is None: push('Listen, only game host can modify', color=(1, 1, 0)); return
    ...
    def do_modify(s):
        ...
        try: name = lmao[dux]
            ...
            error('You what bro?')
            return
        try: a = bot_texture[bot_name.index(val_arr[drux][1])]
        except TypeError:
            error('It\'s dead.')
            return
        ...
        nice2_view = bui.buttonwidget(parent=mid_widget,
                       ...
                       button_type='square',
                       color=(1, 1, 1),
                       texture=bui.gettexture(bot_texture[bot_name.index(lmao_chars[dux])]+'Icon'),
                       mask_texture=bui.gettexture('characterIconMask'),
        ...
        bui.buttonwidget(edit=nice2_view, tint_texture=bui.gettexture(bot_texture[bot_name.index(val_arr[drux][1])]+'IconColorMask'))
        bui.buttonwidget(edit=nice2_view, tint_color=val_arr[drux][6], tint2_color=val_arr[drux][11])

        try: pus = lmao_bots[dux].node.position
        except AttributeError:
    ...
    def do_modify2(s):
        ...
        for i in range(len(attrs)):
            bui.textwidget(parent=mud_sub,
                           ...
                           h_align='left',
                           v_align='center',
                           on_activate_call=bs.Call(s.welp, i) if i not in not_editable else bs.Call(s.welp, i, nah=True),
                           selectable=True,
                           autoselect=True,
            ...
            if isinstance(a, bool):
                l = bui.checkboxwidget(parent=mud_sub,
                               ...
                               color=colb,
                               scale=s.scale/2,
                               on_value_change_call=bs.Call(s.check, i, mod=True) if i not in not_editable else bs.Call(s.welp, i, ignore=True),
                               position=(180, cb - (20 * i)))
            elif isinstance(a, tuple) or i == 6 or i == 11 or i == 28:
                k = val_attrs2[i]
    ...
    def gather2(s, ran=False):
        ...
        for i in range(len(ins2)):
            ...
            if bui.Widget.get_widget_type(ins2[i]) == 'checkbox' and ran:
                ...
                for r in range(3): a.append(random.uniform(0.0, 1.0))
                a = (float(a[0]), float(a[1]), float(a[2]))
                bui.buttonwidget(edit=ins2[i], label=f"{str(a[0]+0.01)[:3]} {str(a[1]+0.01)[:3]}, {str(a[2]+0.01)[:3]}", color=(a[0], a[1], a[2]), textcolor=(1-a[0], 1-a[1], 1-a[2]))
                val_attrs2[i] = a
#                bui.buttonwidget(edit=nice2_view, tint_texture=bui.gettexture(val_attrs2[1]+'IconColorMask'))
                bui.buttonwidget(edit=nice2_view, tint_texture=bui.gettexture(bot_texture[bot_name.index(val_attrs2[1])]+'IconColorMask'))
                bui.buttonwidget(edit=nice2_view, tint_color=val2_attrs[6], tint2_color=val2_attrs[11])
        if not ran: s.kill(True, mud_widget, True)
        else: bui.getsound('cashRegister2').play()
    ...
    def custom2_window(s):
        ...
        global cords2
        try: txt = str(cords2[0]) if cords2[0] != 69123 else "0"
        except TypeError:
            cords2 = (0, 0, 0)
            txt = "0"
    ...
    def apply_mods(s):
        ...
        new = val_attrs2
        bot = lmao_bots[dux]
        if cords2[0] != 69123 and do_tp: bot.node.handlemessage(bs.StandMessage(cords2, 0))
    ...
    def set_char(s, bot, char):
        ...
        with ga().context:
            try: pelvis = bs.getmesh(c+'Pelvis')
            except RuntimeError: pelvis = bs.getmesh('kronkPelvis')
    ...
    def play_music(s, default=False):
        global music_dux
        try:
            with ga().context: bs.setmusic(music_type[music_dux] if not default else ga().default_music)
        except AttributeError:
            if not default:
        ...
        if default:
            try: push(f"Now playing default music: {music_name[music_type.index(ga().default_music)]}")
            except AttributeError: push("Unable to get default music\nsince you are not the host\nit resets next game tho", (1, 1, 0))

    def effect_window(s):
        if ga() is None: push('Effect who and how? you are not the host!', color=(1, 1, 0)); return
    ...
    def switch_tab(s, id):
        ...
        for w in effect_sub.get_children(): w.delete()
        if id == "bots":
            effect_bots = True
            try: a = effect_dux
            except NameError: a = 0
    ...
    def load(s, arr, arr2, container, sub, dux=0, mod=2):
        ...
        if len(arr) == 0 or str(ga()) != old_ga and mod == 2: s.inform('Still\nNo bots', effect_sub); return
        ones = []
        for i in range(len(arr)):
            try:
                alive = (arr2[i].node.hurt < 1) if arr2[i] in lmao_bots else arr2[i].actor.node.hurt < 1
            except IndexError:
                s.kill(True, widget)
    ...
    def inform(s, what, where):
        ...
        for i in where.get_children(): i.delete()
    ...
    def do_effect(s):
        ...
        if not s.select_all_bots:
            ...
            try: name = _lmao[_effect_dux]
            except NameError:
                error('Select a bot first' if len(_lmao_bots) else 'When it says no bots\nyet u still click the button??')
                return
            except IndexError: pass
            try:
                hurt = _lmao_bots[_effect_dux].node.hurt if effect_bots else _lmao_bots[_effect_dux].actor.node.hurt
                if hurt == 1:
                    error(f'{_lmao[_effect_dux]} is dead.'); return
            except:
                try: error(f'{_lmao[_effect_dux]} is dead.'); return
                except IndexError: error("No bots"); return
        else:
            _lmao = lmao
            ...
            name = "All bots"
        try: a = effect_indox
        except NameError: effect_indox = 0
        ...
        if not s.select_all_bots:
            ...
            if effect_bots: va = val_arr[_effect_dux]
            bui.buttonwidget(edit=nice2_view, tint_texture=bui.gettexture(effect_texture[_effect_dux]))
        ...
        if s.select_all_bots: return
        if effect_bots: val_attrs2 = val_arr[_effect_dux].copy() # reset to default temp
    ...
    def apply_effects(s):
        ...
        if not s.select_all_bots:
            ...
            bots = lmao_bots
            name = "All Bots"
        try: em = effect_message[n]
        except IndexError: em = None
        with ga().context:
            for bot in bots:
                if em: bot.handlemessage(bs.PowerupMessage(em))
                elif a == 'Shatter': bot.shatter(True)
                elif a == 'Freeze': bot.handlemessage(bs.FreezeMessage())
                elif a == 'Unfreeze': bot.handlemessage(bs.ThawMessage())
                elif a == 'Celebrate': bot.handlemessage(bs.CelebrateMessage())
                elif a == 'Stop Celebrating': bot.handlemessage(bs.CelebrateMessage(duration=0.0001))
                elif a == 'Kill':
                    try: bot.handlemessage(bs.DieMessage())
                    except: pass # bot is dead or so
                elif a == 'Infinite Curse':
                    if bot._cursed: bot.handlemessage(bs.PowerupMessage('health'))
                    bot.curse(); s.spam_curse(bot)
                elif a == 'Super Speed': bot.node.hockey = True
                elif a == 'Normal Speed': bot.node.hockey = False
                elif a == 'Invincible': bot.node.invincible = True
                elif a == 'Beatable': bot.node.invincible = False
                elif a == 'Sleep': bot._knocked = True; s.spam_knock(bot)
                elif a == 'Wake Up': bot._knocked = False
                elif a == 'Super Punch': s.give_sp(bot)
                elif a == 'Normal Punch': bot._punch_power_scale = 1.2; bot._punch_cooldown = 400
                elif a == 'Fly Jumps':
                    if effect_bots: bot.on_jump_press = s.spaz_bot_fly(bot.on_jump_press)
                    else: lmao_players[i].assigninput(ba.InputType.JUMP_PRESS, bs.Call(s.spaz_fly, bot))
                elif a == 'Normal Jumps': bot.on_jump_press = s.spaz_not_fly
                elif a == 'GodMode Preset':
                    bot.node.hockey = True # Speed
                    bot._super = True
                    bot.node.invincible = True # Invincibility
                    s.give_sp(bot) # Super Punch
                    PopupText("I HAVE THE POWER",position=bot.node.position,random_offset=1).autoretain()
                elif a == "Reset All":
                    ...
                    bot._knocked = False
                    bot._punch_power_scale = 1.2; bot._punch_cooldown = 400
                    return
    ...
    def nodetimer(s, time, node):
        with ga().context: bs.timer(time, node.delete)
    ...
    def constant_jump(s, bot):
        ...
        if random.choice([False, False, False, True]): PopupText("Hoppie",position=p2,random_offset=0.3,color=(1,0,1)).autoretain()
        bs.timer((random.choice([0.1, 0.4, 0.7, 0.1]) if p2[1] < 4 else 1.5), bs.Call(s.constant_jump, s, bot))
    ...
    def toxic_celebrate(s):
        for b in Nice.toxic_bots:
            try: p = b.node.position
            except: return # bot is dead
            p2 = (p[0],p[1]-0.2,p[2])
#            PopupText(random.choice(toxic_win),position=p2,random_offset=0.3,color=(0,1,1)).autoretain()
    ...
    def make_toxic(s, bot):
        with ga().context:
            if not bot.exists(): return
            p = bot.node.position
            if bot.node.hurt > 0.5: bot.handlemessage(bs.PowerupMessage('shield'))
            p2 = (p[0],p[1]-0.2,p[2])
#            PopupText(random.choice(toxic),position=p2,random_offset=0.3,color=(1,0,0)).autoretain()
    ...
    def spam_knock(s, bot):
        with ga().context:
            if not bot.exists() or not bot._knocked: return
    ...
    def spam_curse(s, bot):
        with ga().context:
            if not bot.exists() or not bot._cursed: return
            bot.handlemessage(bs.PowerupMessage('health'))
    ...
    def update_alive_bots(s):
        ...
        for p in range(len(lmao)):
            ...
            except AttributeError: continue # vanished

    # fly override
    def spaz_bot_fly(s, self):
        def wrapper(b):
            is_moving = abs(b.node.move_up_down) >= 0.01 or abs(b.node.move_left_right) >= 0.01
            if not b.node.exists(): return
            t = ba.apptime()
            b.last_jump_time_ms = -9999
            if t - b.last_jump_time_ms >= b._jump_cooldown:
                b.node.jump_pressed = True
                if b.node.jump_pressed:
                    v = b.node.velocity; v1 = v[0]; v2 = v[1]; v3 = v[2]
                    p = b.node.position; p1 = p[0]; p2 = p[1]; p3 = p[2]
                    r = b.node.run
                    b.node.handlemessage("impulse",p1,0.0+p2,p3,v1,v2,v3,0*r,0*r,0,0,v1,v2,v3)
                    b.node.handlemessage("impulse",p1,3.6+p2,p3,v1,v2,v3,0*r,0*r,0,0,v1,v2,v3)
                    b.node.handlemessage('impulse',p1,p2+0.001,p3,0,0.2,0,200,200,0,0,0,5,0)
                b.last_jump_time_ms = t
            b._turbo_filter_add_press('jump')
        return wrapper

    def spaz_fly(s, _bot):
        if not _bot.node.exists(): return
    ...
    def bombdown(s, b=1):
        ...
        if b:
            bomb_down = True
            with ga().context: bs.timer(0.1, bs.Call(s.bombdown, 0))
        else: bomb_down = False

    def key(s, i):
        global lmao_bots, move_on, currently_dux, bomb_control, lmao, currently_txt
        global alive_bots, alive_arr, bomb_down
        try: bot = lmao_bots[currently_dux]
        except IndexError: push(f"no {currently_dux}")
        with ga().context:
            if i > 2:
                if bomb_control and not bomb_down:
                    s.bombdown()
                    if len(alive_bots) == 1: return # blud only has 1 bot
                    currently_dux += 1
                    if currently_dux  == len(lmao): currently_dux = 0
                    for a in range(len(lmao_bots)):
                        try:
                            if lmao_bots[currently_dux].node.hurt == 1: currently_dux += 1 # dead
                        except AttributeError: currently_dux += 1 # vanished
                    if currently_dux == len(lmao) + 1: currently_dux = 0
                    ...
                    s.hl2(lmao_bots[currently_dux].node, True)
                    return
                elif bomb_down: push('too fast'); return
                bot.on_bomb_press()
                bot.on_bomb_release()
            elif i > 1:
                try: bot.on_jump_press()
                except TypeError: bot.on_jump_press(bot)
                bot.on_jump_release()
            elif i:
                bot.on_punch_press()
    ...
    def pls_move(s):
        ...
        if s.thex and s.they:
            try: b = lmao_bots[currently_dux]
            except IndexError: return # control was stopped
            except TypeError: return # bot died lmao
...
#                error("an error occured, falling back and stopping control\nthis is a failsafe.")
            ...
            if not Nice.lite_mode and Nice.animate_camera:
                try: _ba.set_camera_target(p[0], p[1], p[2])
                except UnboundLocalError: s.draw()
        bs.apptimer(0.01, s.pls_move)

    # s.welp bro lmao
    def welp(s, w, nah=False, ignore=None):
        if ignore: nah = ignore
        global attrs
        ...
        welp_widget = bui.containerwidget(parent=bui.get_special_widget('overlay_stack'),
                                          size=(400, 200 if title not in ['custom_name', 'gravity_scale'] else 230),
                                          color=cola,
                                          stack_offset=s.soff,
                                          transition=s.anim_in,
                                          scale=s.scale)
    ...
    def check(s, n, v, mod=False):
        global val_attrs2
        if mod: val_attrs2[n] = v
        else: Nice.val_attrs[n] = v
    def tick(s, n, v):
        if not n: s.auto_spawn_on_random = v
        elif n == 1: s.random_peace = v
    ...
    def spawn(s, i, mod=False):
        ...
        try: bui.textwidget(edit=nn, text=bot_name[i] if mod not in [2 ,3, 69] else effect_name[i] if mod != 69 else drop_name[i])
        except: s.spawn(i, 0)
        bui.buttonwidget(edit=nv, texture=bui.gettexture((bot_texture[i]+'Icon') if mod not in [2, 3, 69] else effect_texture[i] if mod != 69 else drop_texture[i]))
    ...
    def do_spawn(s):
        ...
        if busy:
            if Nice.pause_when_bots: push('Already spawned a bot\nResume first to spawn another\nOr Turn off pause from Config', color=(1, 1, 0))
            else: push('too fast', color=(1, 1, 0))
            return

        idk = ga()
        if str(idk) != old_ga: on_ga_change()
        with idk.context:
            ...
            for k in ga().players:
                if k.sessionplayer.inputdevice.client_id == -1:
                    p = k
                    lmao.append(random.choice(random_bot_names).replace('#', str(len(lmao)))) #NO_BOT)
                    CustomBot.set_up(attrs, Nice.val_attrs)
            try:
                p.customdata[lmao[-1]] = CustomBotSet(p)
                p.customdata[lmao[-1]].do_custom()
            except NameError: error("You need to be in game to spawn bots"); busy = False

    # know where the hell are cords
    def show_in_game(s, mode=0, mod=False):
        global cords, lmao_bots, move_on, cords2
        if mod is True: co = cords2
        elif mod is False: co = cords
        else: co = Nice.drop_cords
        if co[0] == 69123:
        ...
        with ga().context:
            if mode:
                ...
                if random.randint(1, 10) == 10: push('Tip: wait for some particles to die if line wasn\'t drawn')
    ...
    def hl4(s, bot):
        ...
        def hl4_on(bot, old):
            if w(): bot.node.color = old
        # that spaz is goin blinking fr
        if Nice.lite_mode: return
    ...
    def reset_bright_bots(s):
        ...
        for b in lmao_bots:
            ...
            if c[0] >= 5: b.node.color = (c[0]-5, c[1]-5, c[2]-5)

        try:
            for b in lmao_players:
                try: c = b.actor.node.color
                except: continue
                if c[0] >= 5: b.actor.node.color = (c[0]-5, c[1]-5, c[2]-5)
    ...
    def hl3(s, i, set=True):
        ...
        if Nice.lite_mode: return
        if i is not None:
            try: bot = lmao_bots[i].node if effect_bots else lmao_players[i].actor.node
            except IndexError: return
            old = bot.color
            if set: bot.color = (old[0]+5, old[1]+5, old[2]+5)
    ...
    def use_my_pos(s, c2=False):
        ...
        if c2 is True: cords2 = s.get_my_pos()
        elif c2 is False: cords = s.get_my_pos()
        else: Nice.drop_cords = s.get_my_pos()
        if s.get_my_pos(): s.update_cords_view(c2)
        else: error('You are not in game')
    ...
    def update_cords_view(s, c2=False):
        ...
        try: bui.buttonwidget(edit=cords2_view if c2 is True else cords_view if c2 != 69 else s.drop_where, label=f"X: {c[0]}\nY: {c[1]}\nZ: {c[2]}" if c[0] != 69123 else 'Where To\nTeleport?' if c2 != 69 else "Where To\nDeploy?")
        except TypeError: error("Join the game first bruh")

    # math is OP after all
    def draw_line(s, c, me):
        def gd(c, me): return ((c[0] - me[0])**2 + (c[1] - me[1])**2 + (c[2] - me[2])**2)**0.5
        d = gd(c, me); n = int(d); pol = []
        for i in range(n):
            t = i / (n - 1)
            x = c[0] + t * (me[0] - c[0])
            y = c[1] + t * (me[1] - c[1])
    ...
    def are_close(s, p1, p2, sus):
        try: d = ((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2 + (p1[2] - p2[2])**2)**0.5
        except TypeError: return 1
        if d < sus: return 2
        else: return 3

    def restore_sp(s, bot):
        if not bot._super: return
        if bot._has_boxing_gloves: return
...
class NicePick2(bui.Window):
    ...
    def color_picker_selected_color(s, picker, c):
        ...
        val_attrs2[hmm2] = c
        bui.buttonwidget(edit=ins2[hmm2], label=f"{str(c[0]+0.01)[:3]} {str(c[1]+0.01)[:3]}, {str(c[2]+0.01)[:3]}", color=c, textcolor=(1-c[0], 1-c[1], 1-c[2]))
...
class Picker(popup.PopupWindow):
    def __init__(s, mod=0):
        ...
        count = len(bot_texture) if mod not in [2, 69] else len(effect_texture) if mod != 69 else len(drop_texture)
        columns = 3
        ...
        s._subcontainer = bui.containerwidget(parent=s._scrollwidget, size=(s._sub_width, s._sub_height), background=False)
        ...
        for y in range(rows):
            for x in range(columns):
                ...
                try: icon = bui.gettexture(bot_texture[index] + 'Icon') if mod not in [2, 69] else bui.gettexture(effect_texture[index]) if mod != 69 else bui.gettexture(drop_texture[index])
                except IndexError: return
                btn = bui.buttonwidget(
                    ...
                    autoselect=True,
                    texture=icon,
                    tint_texture=(bui.gettexture(bot_texture[index]+'IconColorMask') if mod not in [2, 69] else None),
                    tint_color=val_attrs2[6] if mod == 1 else None if mod in [2, 69] else Nice.val_attrs[6],
                    tint2_color=val_attrs2[11] if mod == 1 else None if mod in [2, 69] else Nice.val_attrs[11],
                    color=(1, 1, 1),
                    mask_texture=mask_texture,
                    label='',
                    on_activate_call=bs.Call(s.ok, index, mod))
                bui.widget(edit=btn, show_buffer_top=60, show_buffer_bottom=60)
                name = bot_name[index] if mod not in [2, 69] else effect_name[index] if mod != 69 else drop_name[index]
                bui.textwidget(parent=s._subcontainer,
                               ...
                               v_align='center')
                index += 1
                if index >= len(bot_texture if mod not in [2, 69] else effect_texture if mod != 69 else drop_texture): break # brb
            if index >= len(bot_texture if mod not in [2, 69] else effect_texture if mod != 69 else drop_texture): break # bye bye
...
class TexturePicker(popup.PopupWindow):
    def __init__(s):
        ...
        for y in range(rows):
            for x in range(columns):
                pos = (x * (bw + 2 * bbh) + bbh, s._sub_height - (y + 1) * (bh + 2 * bbv) + 12)
                try: icon = bui.gettexture(all_texture[index])
                except IndexError: return
                btn = bui.buttonwidget(
                    parent=s._subcontainer,
                    button_type='square',
                ...
                bui.textwidget(parent=s._subcontainer,
                               ...
                               v_align='center')
                index += 1
                if index >= len(all_texture): break
            if index >= len(all_texture): break # bye bye
...
node_attrs = ['gotta clean this code soon', 'gravity_scale', 'sticky', 'reflection=\'powerup\'', "reflection='soft'", "reflection_scale"]
...
for i in range(len(nah_uh)): nah_uh.append("") # 50% chance to say nothing

def meow_patch(og):
    def wrapper(self, msg: Any) -> Any:
#        print(msg)
        if isinstance(msg, bs.DieMessage):
            if msg.how == bs.DeathType.IMPACT and self.node.getdelegate(object) not in Nice.toxic_bots: Nice.toxic_celebrate(Nice)
            global on_control
            ...
            with ga().context: bs.timer(0.001, bs.Call(Nice.phew, Nice, pos))
            return
            ...
            if on_control: push('Gloves have canceled your Super Punch effect\nDon\'t worry, restoring Super Punch')
        ...
        try: return og(self, msg)
        except: pass # safe mines are gay
    return wrapper
...
class CustomBot(SpazBot):
    ...
    def set_up(cls, attrs, val_attrz):
        ...
        for key, value in dict.items():
            if key == 'character': lmao_chars.append(value)
            if key in ATK:
                if key == ATK[0]:
                    LAH.append(value)
                if key == ATK[1]: LAP.append(value)
                if key == ATK[2]: LAB.append(value)
                if key == ATK[3]: LAF.append(value)
            elif key == 'custom_name':
                t = value
                if value == '%': t = val_attrz[attrs.index('character')]
                elif value == '$':
                    t = lmao[-1] # NO_BOT
                nice_custom_text = t
            elif key == 'custom_name_color': nice_custom_color = value
            else: setattr(cls, key, value)
            ...
            if self.node.getnodetype() == 'spaz':
                ...
                if isinstance(s, SpazBot) and s in lmao_bots:
                    ...
                    if j != 69123:
                        ...
                        if not s._dead:
                            move_on += 1
                            if Nice.notify_bot_ded: push(f'{lmao[lmao_bots.index(s)]} has died!', color=(1, 0, 1))
        ...
        for n in nuds:
            if n.getnodetype() == 'spaz':
                ...
                if isinstance(s, SpazBot):
                    ...
                    if bowl: host = player_pts[-1]
                    else: not_host.append(player_pts[-1])

        for bot in bot_list:
            if bot not in lmao_bots: lmao_bots.append(bot)
            i = lmao_bots.index(bot)
            if not LAH[i]: player_pts.remove(host)
            if not LAP[i]: player_pts = [k for k in player_pts if k not in not_host]
            if not LAB[i]: player_pts = [k for k in player_pts if k not in bad_bots]
            if not LAF[i]: player_pts = [k for k in player_pts if k not in good_bots]
            bot.set_player_points(player_pts)
            bot.update_ai()

        for bot in self.get_living_bots():
            if bot not in lmao_bots2 and bot not in lmao_bots: lmao_bots2.append(bot)

    def setup_custom(self, spaz) -> None:
        spaz.source_player = self.source_player
        self.set_custom_text(spaz)
        for i in Nice.pending:
            if i == "sp": Nice.give_sp(Nice, spaz)
            if i == "speed": spaz.node.hockey = True
            if i == "constant_heal": Nice.constant_heal(Nice, spaz)
            if i == "toxic": Nice.toxic_bots.append(spaz); Nice.make_toxic(Nice, spaz)
            if i == "constant_jump": spaz.on_jump_press = Nice.spaz_bot_fly(Nice, spaz.on_jump_press); Nice.constant_jump(Nice, spaz)
            if i == "infinite_curse":
                if spaz._cursed: spaz.handlemessage(bs.PowerupMessage('health'))
                spaz.curse(); Nice.spam_curse(Nice, spaz)
...
all_texture = [
'achievementBoxer', 'achievementCrossHair', 'achievementDualWielding', 'achievementEmpty',
'achievementFlawlessVictory', 'achievementFootballShutout', 'achievementFootballVictory', 'achievementFreeLoader',
'achievementGotTheMoves', 'achievementInControl', 'achievementMedalLarge', 'achievementMedalMedium',
'achievementMedalSmall', 'achievementMine', 'achievementOffYouGo', 'achievementOnslaught',
'achievementOutline', 'achievementRunaround', 'achievementSharingIsCaring', 'achievementStayinAlive',
'achievementSuperPunch', 'achievementTNT', 'achievementTeamPlayer', 'achievementWall',
'achievementsIcon', 'actionButtons', 'actionHeroColor', 'actionHeroColorMask',
'actionHeroIcon', 'actionHeroIconColorMask', 'advancedIcon', 'agentColor',
'agentColorMask', 'agentIcon', 'agentIconColorMask',
'aliColor', 'aliColorMask', 'aliIcon',
'aliIconColorMask', 'aliSplash', 'alienColor', 'alienColorMask',
'alienIcon', 'alienIconColorMask', 'alwaysLandBGColor', 'alwaysLandLevelColor',
'alwaysLandPreview', 'analogStick', 'arrow', 'assassinColor',
'assassinColorMask', 'assassinIcon', 'assassinIconColorMask', 'audioIcon',
'backIcon', 'bar', 'bearColor', 'bearColorMask',
'bearIcon', 'bearIconColorMask', 'bg', 'bigG',
'bigGPreview', 'black', 'bombButton', 'bombColor',
'bombColorIce', 'bombStickyColor', 'bonesColor', 'bonesColorMask',
'bonesIcon', 'bonesIconColorMask', 'boxingGlovesColor', 'bridgitLevelColor',
'bridgitPreview', 'bunnyColor', 'bunnyColorMask', 'bunnyIcon',
'bunnyIconColorMask', 'buttonBomb', 'buttonJump', 'buttonPickUp',
'buttonPunch', 'buttonSquare', 'chTitleChar1', 'chTitleChar2',
'chTitleChar3', 'chTitleChar4', 'chTitleChar5', 'characterIconMask',
'chestIcon', 'chestIconEmpty', 'chestIconMulti', 'chestOpenIcon',
'circle', 'circleNoAlpha', 'circleOutline', 'circleOutlineNoAlpha',
'circleShadow', 'circleZigZag', 'coin', 'controllerIcon',
'courtyardLevelColor', 'courtyardPreview', 'cowboyColor', 'cowboyColorMask',
'cowboyIcon', 'cowboyIconColorMask', 'cragCastleLevelColor', 'cragCastlePreview',
'crossOut', 'crossOutMask', 'cursor', 'cuteSpaz',
'cyborgColor', 'cyborgColorMask', 'cyborgIcon', 'cyborgIconColorMask',
'doomShroomBGColor', 'doomShroomLevelColor', 'doomShroomPreview', 'downButton',
'egg1', 'egg2', 'egg3', 'egg4',
'eggTex1', 'eggTex2', 'eggTex3', 'empty',
'explosion', 'eyeColor', 'eyeColorTintMask', 'file',
'flagColor', 'flagPoleColor', 'folder', 'fontBig',
'fontExtras', 'fontExtras2', 'fontExtras3', 'fontExtras4',
'fontSmall0', 'fontSmall1', 'fontSmall2', 'fontSmall3',
'fontSmall4', 'fontSmall5', 'fontSmall6', 'fontSmall7',
'footballStadium', 'footballStadiumPreview', 'frameInset', 'frostyColor',
'frostyColorMask', 'frostyIcon', 'frostyIconColorMask', 'fuse',
'gameCenterIcon', 'gameCircleIcon', 'gladiatorColor', 'gladiatorColorMask',
'gladiatorIcon', 'gladiatorIconColorMask', 'glow', 'googlePlayAchievementsIcon',
'googlePlayGamesIcon', 'googlePlayLeaderboardsIcon', 'googlePlusIcon', 'googlePlusSignInButton',
'graphicsIcon', 'heart', 'hockeyStadium', 'hockeyStadiumPreview',
'iconOnslaught', 'iconRunaround', 'impactBombColor', 'impactBombColorLit',
'inventoryIcon', 'jackColor', 'jackColorMask', 'jackIcon',
'jackIconColorMask', 'jumpsuitColor', 'jumpsuitColorMask', 'jumpsuitIcon',
'jumpsuitIconColorMask', 'kronk', 'kronkColorMask', 'kronkIcon',
'kronkIconColorMask', 'lakeFrigid', 'lakeFrigidPreview', 'lakeFrigidReflections',
'landMine', 'landMineLit', 'leaderboardsIcon', 'leftButton',
'levelIcon', 'light', 'lightSharp', 'lightSoft',
'lock', 'logIcon', 'logo', 'logoEaster',
'mapPreviewMask', 'medalBronze', 'medalComplete', 'medalGold',
'medalSilver', 'melColor', 'melColorMask', 'melIcon',
'melIconColorMask', 'menuBG', 'menuButton', 'menuIcon',
'meter', 'monkeyFaceLevelColor', 'monkeyFacePreview', 'multiplayerExamples',
'natureBackgroundColor', 'neoSpazColor', 'neoSpazColorMask', 'neoSpazIcon',
'neoSpazIconColorMask', 'nextLevelIcon', 'ninjaColor', 'ninjaColorMask',
'ninjaIcon', 'ninjaIconColorMask', 'nub', 'null',
...
'ouyaAButton', 'ouyaIcon', 'ouyaOButton', 'ouyaUButton',
'ouyaYButton', 'penguinColor', 'penguinColorMask', 'penguinIcon',
'penguinIconColorMask', 'pixieColor', 'pixieColorMask', 'pixieIcon',
'pixieIconColorMask', 'playerLineup', 'powerupBomb', 'powerupCurse',
'powerupHealth', 'powerupIceBombs', 'powerupImpactBombs', 'powerupLandMines',
'powerupPunch', 'powerupShield', 'powerupSpeed', 'powerupStickyBombs',
'puckColor', 'rampageBGColor', 'rampageBGColor2', 'rampageLevelColor',
'rampagePreview', 'reflectionChar_+x', 'reflectionChar_+y', 'reflectionChar_+z',
'reflectionChar_-x', 'reflectionChar_-y', 'reflectionChar_-z', 'reflectionPowerup_+x',
'reflectionPowerup_+y', 'reflectionPowerup_+z', 'reflectionPowerup_-x', 'reflectionPowerup_-y',
'reflectionPowerup_-z', 'reflectionSharp_+x', 'reflectionSharp_+y', 'reflectionSharp_+z',
'reflectionSharp_-x', 'reflectionSharp_-y', 'reflectionSharp_-z', 'reflectionSharper_+x',
'reflectionSharper_+y', 'reflectionSharper_+z', 'reflectionSharper_-x', 'reflectionSharper_-y',
'reflectionSharper_-z', 'reflectionSharpest_+x', 'reflectionSharpest_+y', 'reflectionSharpest_+z',
'reflectionSharpest_-x', 'reflectionSharpest_-y', 'reflectionSharpest_-z', 'reflectionSoft_+x',
'reflectionSoft_+y', 'reflectionSoft_+z', 'reflectionSoft_-x', 'reflectionSoft_-y',
'reflectionSoft_-z', 'replayIcon', 'rgbStripes', 'rightButton',
'robotColor', 'robotColorMask', 'robotIcon', 'robotIconColorMask',
'roundaboutLevelColor', 'roundaboutPreview', 'santaColor', 'santaColorMask',
'santaIcon', 'santaIconColorMask', 'scorch', 'scorchBig',
'scrollWidget', 'scrollWidgetGlow', 'settingsIcon', 'shadow',
'shadowSharp', 'shadowSoft', 'shield', 'shrapnel1Color',
'slash', 'smoke', 'softRect', 'softRect2',
'softRectVertical', 'sparks', 'star', 'startButton',
'stepRightUpLevelColor', 'stepRightUpPreview', 'storeCharacter', 'storeCharacterEaster',
'storeCharacterXmas', 'storeIcon', 'superheroColor', 'superheroColorMask',
'superheroIcon', 'superheroIconColorMask', 'textClearButton', 'thePadLevelColor',
'thePadPreview', 'ticketRoll', 'ticketRollBig', 'ticketRolls',
'tickets', 'ticketsMore', 'tipTopBGColor', 'tipTopLevelColor',
'tipTopPreview', 'tnt', 'touchArrows', 'touchArrowsActions',
'towerDLevelColor', 'towerDPreview', 'treesColor', 'trophy',
'tv', 'uiAtlas', 'uiAtlas2', 'upButton',
'usersButton', 'vrFillMound', 'warriorColor', 'warriorColorMask',
'warriorIcon', 'warriorIconColorMask', 'white', 'windowHSmallVMed',
'windowHSmallVSmall', 'wings', 'witchColor', 'witchColorMask',
'witchIcon', 'witchIconColorMask', 'wizardColor', 'wizardColorMask',
'wizardIcon', 'wizardIconColorMask', 'wrestlerColor', 'wrestlerColorMask',
'wrestlerIcon', 'wrestlerIconColorMask', 'zigZagLevelColor', 'zigzagPreview',
'zoeColor', 'zoeColorMask', 'zoeIconColorMask']
